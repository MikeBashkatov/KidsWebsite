    ...
    var getNotesThisMonth = function() {
		...
		var triggerAction = function() {
		    ...
		    $('body').on('click', '#calendarClick', function(){
		        ...
		        //Get date of the clicked cell
				var selectedDate = $(this).data('date');
				var selectedMonth = $(this).data('month');
				var selectedYear = $(this).data('year');

				//Array with TV shows names
                var names = ['Aaahh!!! Real Monsters',
                ...
                // GEt a random show from my array
                let rand = names[Math.floor(Math.random() * names.length)];
                $('#modal_header').text('GREAT NEWS!');
                $('#dateTime').text(`Schedule for ${selectedDate}/${selectedMonth}/${selectedYear}`);
                $('#schedule').text(`We will be glad to present you our new TV Show!`);
                $('#tv_show_name').text(rand);
		...
		return {
			build: function() {
                ...
                if (windowsize < 992) {
                    //if the window is greater than 992px wide use dayNames otherwise use dayShortNames
                        settings.dayUseShortName = true;
                }

                else {
                    settings.dayUseShortName = false;
                }
